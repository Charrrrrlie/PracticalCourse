...
void Bundle::calculate(int times)
{
	...
	Eigen::MatrixXd A=Eigen::MatrixXd::Zero(pt_num * 2, pt_num * 3 + pts.size() * 6);
	...
	for (int t = 0; t < times; t++)
	{
		...
		for (int i = 0; i < point.size(); i++)
		{
			...
			for (int j = 0; j < my.size();j++)
			{
				...
				a = diff(temp_pt, element[my[j].left]);   // dx dy dz dphi domega dkappa dxs dys dzs
...
Eigen::MatrixXd Bundle::adj(Eigen::MatrixXd A, Eigen::MatrixXd L)
{
	...
	param = A.sparseView();
	Eigen::SimplicialCholesky<Eigen::SparseMatrix<double>> solver (param.transpose() * param);

	ans=solver.solve(param* L);
	cout << ans;
